(function () {
    ...
    function cloneDefaultCareerState() {
        return JSON.parse(JSON.stringify(defaultCareerState));
    }

    function parseCareerState(root) {
        const rawState = root?.getAttribute("data-career-state");
        if (!rawState) {
        ...
        try {
            ...
            return {
                ...cloneDefaultCareerState(),
                ...parsed,
                study: {
                    ...cloneDefaultCareerState().study,
                    ...(parsed.study || {}),
                },
            };
        } catch (error) {
    ...
    function initCareerPage(scope = document) {
        const root = scope.matches?.("[data-career-root]")
            ? scope
            : scope.querySelector?.("[data-career-root]");

        if (!root) {
            return;
        }

        let careerState = parseCareerState(root);

        const pipelineForm = root.querySelector("#pipelineForm");
        const pipelineFormShell = root.querySelector("#pipelineFormShell");
        const togglePipelineFormButton = root.querySelector("#togglePipelineForm");
        const pipelineList = root.querySelector("#pipelineList");
        const skillList = root.querySelector("#skillList");
        const saveSkillsButton = root.querySelector("#saveSkillsButton");
        const studyForm = root.querySelector("#studyForm");
        const studyHoursInput = root.querySelector("#studyHoursInput");
        const targetHoursInput = root.querySelector("#targetHoursInput");
        const subjectInput = root.querySelector("#subjectInput");
        const addSubjectButton = root.querySelector("#addSubjectButton");
        const studySubjectChips = root.querySelector("#studySubjectChips");
        const studySubjectList = root.querySelector("#studySubjectList");
        const todayStudyValue = root.querySelector("#todayStudyValue");
        const weeklyStudyValue = root.querySelector("#weeklyStudyValue");
        const targetStudyValue = root.querySelector("#targetStudyValue");
        const careerFeedback = root.querySelector("#careerFeedback");
        const aiPrepResponse = root.querySelector("#aiPrepResponse");
        const aiPrepButtons = Array.from(root.querySelectorAll("[data-ai-prompt]"));

        function persistLocalState() {
            try {
                root.setAttribute("data-career-state", JSON.stringify(careerState));
            } catch (error) {
                // Ignore local serialization issues and keep the UI responsive.
            }
        }
        ...
        function addSubjectToStudy(subjectValue) {
            ...
            const exists = Array.from(catalog).some((item) => item.toLowerCase() === subject.toLowerCase());
            if (exists) {
                return false;
            }
